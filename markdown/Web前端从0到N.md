...
在我加入公司前(2016年初), 软件研发部门的技术栈是围绕着 Java 6 的.
...
算然是创建一个几人的小部门, 也需要领导支持.
这一点很重要, 在繁琐的公司流程中, 如果领导不支持根本无法推进.
...
把设计部里几名网页开发转到Web前端部门下(招聘太慢, 先从其它部门借调).

紧接着的问题是: 老兵遇上新领导, 怎么管理?
我的经验: 最好的方法是"真诚"
...
3年多时间, 前后6名前端 + 20多Java + 3名PHP 参与过Web前端代码编写或维护.
积累下来的前端代码.
...
其次Web前端想要独立, 关键在于代码的控制权. 软件开发过程中, 代码就是一切.

旧的开发流程给 产品经理/后台开发/测试团队 产生了固定印象:
...
* 产品经理/测试团队 熟悉旧的工作流程, 一时难以改变

针对以上问题, 解决问题需要的工作方式是:

* 先清理积压工作, 不要让自己陷入 `忙 -> 将就 -> 问题多 -> 更忙` 这种尴尬
* 申请硬件资源, 有前端自己的服务器, 搭建前端专用辅助开发服务
* 多次的技术分享(刷存在感), 以前端技术为主, 欢迎其它部门同事参加
...
实际上, 在这个阶段出现有人掉队的情况. 对于技术能力跟不上的同事.
不勉强保留岗位, 不能为了迁就个别同事而放弃对前端的技术改进.

在这一转变过程, 特别考验技术经理的综合能力, 怎样才能稳稳的推进技术变革.

* 团队 - 招聘, 通过招聘建设一个梯队型的团队(全精英团队不现实), 看重学习能力.
* 团队 - 优化, 给所有人锻炼新技能的机会. 实在跟不上, 也不勉强保留岗位名额.
* 任务分配 - 轮岗, 交叉工作内容. 让所有人熟悉所有项目, 掌握每个项目的技术能力.
...
对Web前端的再次技术改造, 目标是做出一个 完整的/全面的 最佳实践方案.

对于引入 Webpack 框架, 并不是很有把握, 并且冒着相当大的风险. 那时感觉自己在"玩火".
...
> // 清理编译结果, 删除 build/dist/cdn 目录
> npm run clean 

> // 完整编译某一个项目的全部页面
> npm run build:[project] 

> // 编译某一个项目的公共库, 多页面编译时, 节省时间
> npm run build:[project]:common_js 
...
##### 请求数据流向(模拟API路径)
    ...
    指向本机磁盘路径
        |
        V
    [磁盘]:/workspace/fake-api/project/paths*
        |
        V
       END
